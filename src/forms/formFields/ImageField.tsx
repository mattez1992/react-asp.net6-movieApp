...
export default function ImageField(props: imageFieldProps) {
    ...
    const handleOnChange = (eventsArgs: ChangeEvent<HTMLInputElement>) => {
        if (eventsArgs.currentTarget.files) {
            ...
            if (file) {
                toBase64(file).then((img64: string) => setImageBase64(img64)).catch(error => console.error(error));
    ...
    return (
        <div>
            <label htmlFor="image">{props.displayName}</label>
            ...
            {imageBase64 ?
                <div>
                    <div style={divStyle}>
                        <img src={imageBase64} alt="selected" style={imgStyle} />
                    </div>
                </div>
                : null}
            {imageURL ?
                <div>
                    <div style={divStyle}>
                        <img src={imageURL} alt="selected" style={imgStyle} />
                    </div>
                </div>
                : null}
        </div >
